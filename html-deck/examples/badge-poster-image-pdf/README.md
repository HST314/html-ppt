...
这是 `image-pdf` 路线的可复跑真实项目。`images/new-pack/` 保存 5 张项目原图，`descriptions/` 保存对应图片 MD 描述；`visual_language.json` 是从 MD 编译出的唯一视觉语言。`outline.json` 经绑定脚本整体替换旧元素语义后，生成 15 页 PDF、逐页图片和 QA v2.5 报告。
...
完整回归（含 MD 语言契约、QA v2.5 与四组同步攻击）：`python3 scripts/test_image_pdf.py`。
