...
<!-- TASK-018: 新增 §9——全 deck 视觉结构配额（渲染级结构签名四元组 + 相同/高度相似 ≤2 页，转场页除外；终审强制检查口径见 final-quality-check.md D1） -->
...
<!-- TASK-018: 新增整节——全 deck 视觉结构配额（根因：连页签名禁令只管相邻页，缺全 deck 总量配额，导致多页渲染结构同质） -->
## 9. 全 deck 视觉结构配额（强制 · 渲染级判定）

**规则**：全 deck 内，视觉结构相同或高度相似的页面**不得超过 2 页**（转场页——封面 / 章节 / 目录 / 尾页 closing——除外）。超过即 QA 判失败，必须对超出页改结构。

**判定单位：渲染级视觉结构签名（四元组）**。不以 pattern / 变体名为准（名义变体可能渲染同构），以实际渲染结构为准，逐页登记四元组：

| 维度 | 取值示例 | 说明 |
| --- | --- | --- |
| ① 标题位置 | 通栏顶 / 左上 / 居中 | h1/h2 在页面网格中的落位 |
| ② 内容区切分 | 单栏堆叠 / 双栏 5:7 / 双栏 7:5 / 三列 / 2×2 格阵 / 2×3 格阵 / 横向链 / 中心辐射 / 上核下支 / 上支下核 / 分层条 / 对比双栏 | 主体区域实际的空间切分方式（§1–§5 子系统组合） |
| ③ 焦点位置 | 首卡 / 右栏 / 中心核 / 底部核 / 终点节点 / 大数字 | 视觉重心落点 |
| ④ 模块节奏 | 卡组 / 节点链 / 层条 / 格阵 / 对照栏 / 数字面板 | 信息模块的形态与排布节奏 |

**相似判定（可执行）**：两页四元组有 **≥3 维相同** 即判"视觉结构相同或高度相似"，计入同一签名配额。

**伪变体不算结构差异**：仅改配色、描边、边框、圆角、阴影而四元组不变的"变体"，不视为结构差异（如 copy 左卡 + visual 右卡 + 底部结论条的双栏骨架，换右边框样式仍是同一签名）。结构差异必须落在四元组至少 2 维上（通常改 ② 内容区切分 + ③ 焦点位置）。

**与其他反同质化规则的关系**（三层互补，都执行，互不替代）：

- 连页签名禁令（`layout-patterns.md` 铁律）：管**相邻页** pattern + 变体不重复——相邻层。
- 同场景签名不得雷同（`design-scenario-system.md` §4 反模式 5）：管**同场景**页——场景层。
- **本节配额**：管**全 deck 渲染级总量**——deck 层，拦截"名义变体不同、渲染结构同构"的隐性同质化。

**落盘与检查**：QA 在 `state/qa_report.md` 终审节逐页登记四元组签名并做全 deck 比对；终审强制检查项与阻断口径见 `final-quality-check.md` D1。
