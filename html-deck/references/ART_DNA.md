...
- 四类角色背景缺任一个，或 HTML 任一页没有项目视觉层：失败。（<!-- TASK-015 -->「HTML 任一页」自 TASK-015 起按内容页/章节页核对，封面/尾页回退经典 deco 版式，见文末说明）
...
用户反馈封面/尾页文字与生成式融合背景对比不足（"融为一体"）。自 TASK-015 起：

- 封面（cover）/尾页（closing）不再注入 project-art 生成背景，恢复 `deco.py` 的 cover_deco/closing_deco 经典高对比版式（深色基底 + 主题色装饰 + 浅色文字）。
- art DNA 仍须生成四类角色背景（schema 与阻断检查不变），其中 content/section 背景照常注入内容页/章节页；cover/closing 背景生成但渲染端不消费。
- QA「项目视觉 DNA 覆盖」门禁适用范围收敛为内容页/章节页（缺一报一，容差不变）。
